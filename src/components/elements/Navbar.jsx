...
const Navbar = () => {
  ...
  return (
    <header
      className={`w-[90%] mx-auto max-w-screen-2xl z-50 ${
        isScrolled ? "absolute" : "fixed"
      } h-28 shadow-md top-0 left-1/2 -translate-x-1/2`}
      style={{ borderBottomRightRadius: "9px" }}
    >
      <div
        className={`${
          isSmallerScreen ? "w-[35%]" : "w-[20%]"
        } px-3 py-5 bg-black clip-custom absolute top-0 h-full left-0`}
        style={{ borderBottomLeftRadius: "9px" }}
      >
        <img
          src={logoIcon}
          className={`${isSmallerScreen ? "w-[92%]" : "w-[80%]"} h-full`}
          alt="logo"
        />
      </div>
      <div
        className="w-[100%] bg-white pl-[20%] text-white h-full"
        style={{
      ...
      >
            ...
            <div className="icons flex justify-start items-center gap-2">
              ...
              <div className="flex justify-start items-center gap-2">
                <div className="text-sm flex items-center gap-1 text-[.7rem]">
                  ...
                  Email: contact@apiero.in
                </div>
              </div>
            </div>
          )}
          <div
            className={`socials flex items-center ${
              isSmallerScreen ? "ml-auto" : ""
            } justify-end gap-4 text-[.9rem]`}
          >
            <h4>Follow us on:</h4>
            <a href="/">
              <FaFacebook />
            </a>
            <a href="https://www.instagram.com/apieromedica/">
              <FaInstagram />
            </a>
            <a href="/">
              <FaTwitter />
        ...
        <div className="header-bottom w-full p-2 h-[61%] flex items-center justify-between">
          ...
          <div className="w-2/5 h-full flex justify-end items-center gap-5 pr-5 ml-auto">
            <Link to="/login">
              <Button
                className="bg-gradient-blue py-5 px-7 text-white rounded-3xl hover:-translate-y-1 transition-all duration-500"
                variant="secondary"
              >
                Login{" "}
              </Button>
